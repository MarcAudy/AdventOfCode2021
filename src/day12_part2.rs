...
fn get_paths_from(cur_node: u8, prev_visited_data: &VisitedData, cave_data: &CaveData) -> Vec<Vec<u8>>
{   
    let mut paths: Vec<Vec<u8>> = Vec::new();
    for connected_node in cave_data.cave_connections.get(&cur_node).unwrap()
    {
        let mut visited_data = VisitedData { 
            visited_nodes: [prev_visited_data.visited_nodes.as_slice(), &[cur_node]].concat(), 
            small_visited_twice: prev_visited_data.small_visited_twice
        };
        
        if *connected_node == cave_data.end_node
        {
            paths.push([visited_data.visited_nodes.as_slice(), &[*connected_node]].concat());
            continue;
        }
        else if cave_data.small_caves.contains(connected_node)
        {
            if prev_visited_data.visited_nodes.contains(connected_node)
            {
                if *connected_node == cave_data.start_node || visited_data.small_visited_twice
                {
                    continue;
                }
                visited_data.small_visited_twice = true;
            }
        }
        paths.append(&mut get_paths_from(*connected_node, &visited_data, cave_data));
    }
...
fn main() -> Result<(), Error> 
{
    ...
    let paths = get_paths_from(cave_data.start_node, &VisitedData::new(), &cave_data);
